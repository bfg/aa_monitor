package Noviforum::Adminalert::Protocol::_HTTPCommon;
...
sub renderDoc {
	my ($self, $pkg) = @_;
	# load renderer class
	eval { require Noviforum::Adminalert::PodRenderer };
	return undef if ($@);

	# render package
	return Noviforum::Adminalert::PodRenderer->new()->render($pkg);
	#my $r = Noviforum::Adminalert::PodRenderer->new();
	#return $r->render($pkg);
}
